...
export default function ProfileView({
  ...
  const handleSubmit = async (formData: FormData) => {
    startTransition(async () => {
      ...
      if (avatarUrl && avatarUrl !== profile.image)
        formData.set("image", avatarUrl);
      if (bannerUrl && bannerUrl !== profile.banner)
        ...
        <div className="lg:col-span-4 space-y-6">
                  ...
                  <div key={match.id} className="flex items-center gap-3 group">
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <a
                        ...
                        className="p-1.5 bg-neutral-800 rounded-lg hover:bg-neutral-700 text-white transition-colors"
                        title="Email"
                      >
          ...
          <div className="bg-neutral-900 rounded-3xl p-6 border border-white/5 shadow-xl">
            ...
            <div className="mt-8 pt-8 border-t border-white/5">
              <h4 className="font-bold text-xs uppercase tracking-widest text-gray-600 mb-4">
                Skills
              </h4>
              <div className="flex flex-wrap gap-2">
                {profile.skills?.split(",").map((skill, i) => (
                  <span
                    key={i}
                    className="px-3 py-1.5 bg-neutral-950 border border-white/10 rounded-lg text-[10px] font-black text-white/80 uppercase tracking-tighter"
                  >
                    {skill.trim()}
                  </span>
                ))}
                      ...
                      className={`flex w-full items-center justify-center gap-3 rounded-[1.5rem] py-5 text-lg font-semibold tracking-[0.01em] font-[family:var(--font-geist-sans)] transition-all ${
                        status === "success"
                          ...
                          : status === "error"
                            ...
                            : "bg-linear-to-r from-zinc-950 via-neutral-900 to-zinc-950 text-white border border-primary/45 hover:border-primary/70 hover:shadow-[0_12px_30px_rgba(234,40,30,0.25)] active:scale-95 disabled:opacity-50"
                      }`}
                    >
                      {status === "success" ? (
                        <>
